...
var defaultIosOutputDir = "./remote/ios/";
var defaultAndroidOutputDir = "./platforms/android/build/outputs/apk/release/";
...
function InitBuildVariables() {
    ...
    if (env == ENV_TYPE.PROD) {
        defaultIosOutputDir = `${defaultIosOutputDir}release/`;
    } else {
        defaultIosOutputDir = `${defaultIosOutputDir}debug/`;
    }
...
gulp.task('remove-androidx-adapter', function (cb) {
    RunTacoCommand('cordova', 'plugin remove cordova-plugin-androidx-adapter', false, cb);
});

gulp.task('remove-whitelist', function (cb) {
    RunTacoCommand('cordova', 'plugin remove cordova-plugin-whitelist', false, cb);
});
...
gulp.task('add-platform-android', function (cb) {
     RunTacoCommand('cordova', 'platform add android@11.0.0', false, cb);
});

gulp.task('add-platform-ios', function (cb) {
    RunTacoCommand('cordova', 'platform add ios@6.3.0', false, cb);
});
...
gulp.task('before-build', function (cb) {
    ...
    var tasks =
        ['clean',
        ['remove-plugins-cordova-plugin-customurlscheme'],
            ['transform-config', 'transform-app-settings', 'transform-build-settings', 'add-plugins-cordova-plugin-customurlscheme', 'transform-firebase-config', 'copy-custom-plugins', 'remove-androidx-adapter', 'remove-whitelist'],
        ];
    ...
    for (var ind = 0; ind < platforms.length; ind++) {
        ...
        if (platform == PLATFORM.IOS) {
            ...
            tasks.push('add-platform-android');
            //tasks.push('install-dependencies-android');
        }
    }
...
gulp.task('build-android', function (cb) {
    RunTacoCommand('cordova', 'build android --release --device', true, cb, false);
});
...
gulp.task('build-ios', function (cb) {
    ...
    RunTacoCommand('cordova', buildCmd, true, cb, true);
});
...
gulp.task('help', function () {
    var helpText = `
=================================================================
   =>   Faculty App Build Tools for AIU and CTU, V 1.0.0    <=
=================================================================
...
gulp build --ctu --platform=android,ios --env=int --outputDir=C:/temp --cleanOutputDir
